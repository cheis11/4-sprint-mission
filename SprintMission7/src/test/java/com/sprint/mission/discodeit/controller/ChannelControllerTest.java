...
class ChannelControllerTest {
  ...
  void updateChannel_success() throws Exception {
    UUID channelId = UUID.randomUUID();
    PublicChannelUpdateRequest request = new PublicChannelUpdateRequest("updated-name", "updated-desc");
    ...
    BDDMockito.given(channelService.update(eq(channelId), any(PublicChannelUpdateRequest.class))).willReturn(dto);
  ...
  void deleteChannel_fail_notFound() throws Exception {
    UUID channelId = UUID.randomUUID();
    BDDMockito.willThrow(new RuntimeException("Channel not found")).given(channelService).delete(channelId);
  ...
  void findAllChannels_success() throws Exception {
    ...
    ChannelDto channel1 = new ChannelDto(UUID.randomUUID(), ChannelType.PUBLIC, "channel1", "설명1", List.of(), null);
    ChannelDto channel2 = new ChannelDto(UUID.randomUUID(), ChannelType.PRIVATE, "channel2", "설명2", List.of(), null);

    BDDMockito.given(channelService.findAllByUserId(userId)).willReturn(List.of(channel1, channel2));
