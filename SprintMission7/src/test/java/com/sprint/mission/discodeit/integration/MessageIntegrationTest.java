...
class MessageIntegrationTest {
  ...
  void createMessage_success() {
    // given
    UserDto user = userService.create(new UserCreateRequest("alice", "alice@example.com", "1234"), java.util.Optional.empty());
  ...
  void updateMessage_success() {
    // given
    UserDto user = userService.create(new UserCreateRequest("bob", "bob@example.com", "1234"), java.util.Optional.empty());
    ChannelDto channel = channelService.create(new PublicChannelCreateRequest("dev", "dev talk"));

    MessageDto message =
        messageService.create(new MessageCreateRequest("Old Content", channel.id(), user.id()), List.of());
  ...
  void deleteMessage_success() {
    // given
    UserDto user = userService.create(new UserCreateRequest("tom", "tom@example.com", "1234"), java.util.Optional.empty());
    ChannelDto channel = channelService.create(new PublicChannelCreateRequest("random", "random chat"));

    MessageDto message =
        messageService.create(new MessageCreateRequest("to be deleted", channel.id(), user.id()), List.of());
  ...
  void getMessage_success() {
    ...
    UserDto user = userService.create(
        new UserCreateRequest("jane", "jane@example.com", "1234"),
        java.util.Optional.empty()
    );
    ChannelDto channel = channelService.create(
        new PublicChannelCreateRequest("notice", "notice board")
    );

    MessageDto message = messageService.create(
        new MessageCreateRequest("check this out", channel.id(), user.id()),
        List.of()
    );
